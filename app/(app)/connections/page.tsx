    ...
    >
      <div className="modal animate-fade-up">
        <div style={{ padding: "24px 24px 0" }}>
          ...
          >
            Connect a PostgreSQL database. Your credentials are encrypted at
            rest.
          </p>
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
          ...
          >
            <RefreshCw
              size={12}
              style={{
                animation: indexing ? "spin 1s linear infinite" : "none",
              }}
            />
    ...
    <div className="card" style={{ padding: 20 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          <div
            className="skeleton"
            style={{ width: 140, height: 16, marginBottom: 6 }}
          />
...
export default function ConnectionsPage() {
  ...
  const { addConnection, removeConnection, setConnections: storeSet } =
    useConnectionStore();

  const isTeam = has?.({ feature: "team_tier" });
  const isPro = has?.({ feature: "pro_tier" });

  const maxConnections = isTeam
    ? Infinity
    : isPro
    ? PLAN_LIMITS.pro
    : PLAN_LIMITS.free;
  ...
  const atLimit =
    !loading && maxConnections !== Infinity && connections.length >= maxConnections;

  const requiredPlan: "pro" | "team" = isPro ? "team" : "pro";
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 14, color: "var(--text-secondary)" }}>
            Manage PostgreSQL database connections. Credentials are encrypted
            end-to-end.
          </p>
        </div>

        {/* Show Add button only when not at limit */}
        {!atLimit && !loading && (
          <button
            className="btn btn-primary"
      ...
      {!loading && maxConnections !== Infinity && (
        <div
          style={{
            display: "flex",
            alignItems: "center",
        ...
        >
          ...
          >
            {connections.length} / {maxConnections} connections
          </span>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {atLimit && (
          <UpgradePrompt
            ...
            description={`You've reached the ${maxConnections}-connection limit on your current plan.`}
            currentCount={connections.length}
            limit={maxConnections as number}
            inline
          />
        )}
      </div>
